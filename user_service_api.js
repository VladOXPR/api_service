...
router.post('/users', async (req, res) => {
  ...
  try {
    ...
    try {
      // Get the next ID by finding MAX(id) + 1 (or 1 if table is empty)
      const maxIdResult = await client.query(
        'SELECT COALESCE(MAX(id::integer), 0) + 1 AS next_id FROM users'
      );
      const nextId = maxIdResult.rows[0].next_id;
      
      // Insert new user with auto-generated ID
      const result = await client.query(
        `INSERT INTO users (id, username, type, created_at, updated_at)
         VALUES ($1, $2, $3, NOW(), NOW())
         RETURNING id, username, type, created_at, updated_at`,
        [nextId, username, type || 'HOST']
      );
